...
class Jdf {
    ...
    function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
        ...
        $T_sec=0;/* <= ÑÝÚ ÎØÇí ÒãÇä ÓÑæÑ ¡ ÈÇ ÇÚÏÇÏ '+' æ '-' ÈÑ ÍÓÈ ËÇäíå */
        ...
        for($i=0; $i<$sl; $i++){
            ...
            switch($sub){
                ...
                case'a':
                    $out.=($date[0]<12)?'Þ.Ù':'È.Ù';
                    break;

                case'A':
                    $out.=($date[0]<12)?'ÞÈá ÇÒ ÙåÑ':'ÈÚÏ ÇÒ ÙåÑ';
                    break;
    ...
    function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
        ...
        for($i=0; $i<$sl; $i++){
            ...
            switch($sub){
                ...
                case'p':
                    $out.=($date[1]<12)?'ÞÈá ÇÒ ÙåÑ':'ÈÚÏ ÇÒ ÙåÑ';
                    break;

                case'P':
                    $out.=($date[1]<12)?'Þ.Ù':'È.Ù';
                    break;

                case'r':
                    $out.=$date[0].':'.$date[2].':'.$date[5].' '.(($date[1]<12)?'ÞÈá ÇÒ ÙåÑ':'ÈÚÏ ÇÒ ÙåÑ');
                    break;
        ...
        foreach($array as $type=>$num){
            ...
            switch($type){
                ...
                case'ss':
                    ...
                    if($xy3==1){
                        $p34='';
                        $k34=array('Ïå','?ÇÒÏå','ÏæÇÒÏå','Ó?ÒÏå','åÇÑÏå','ÇäÒÏå','ÔÇäÒÏå','åÝÏå','åÌÏå','äæÒÏå');
                        $h34=$k34[substr($num,2-$sl,2)-10];
                    }else{
                        $xy4=substr($num,3-$sl,1);
                        $p34=($xy3==0 or $xy4==0)?'':' æ ';
                        $k3=array('','','È?ÓÊ','Ó?','åá','äÌÇå','ÔÕÊ','åÝÊÇÏ','åÔÊÇÏ','äæÏ');
                        $h3=$k3[$xy3];
                        $k4=array('','?˜','Ïæ','Óå','åÇÑ','äÌ','ÔÔ','åÝÊ','åÔÊ','äå');
                        $h4=$k4[$xy4];
                    }
                    $array[$type]=(($num>99)?str_ireplace(array('12','13','14','19','20')
                                ,array('åÒÇÑ æ Ïæ?ÓÊ','åÒÇÑ æ Ó?ÕÏ','åÒÇÑ æ åÇÑÕÏ','åÒÇÑ æ äåÕÏ','ÏæåÒÇÑ')
                                ,substr($num,0,2)).((substr($num,2,2)=='00')?'':' æ '):'').$h3.$p34.$h34.$h4;
                    break;

                case'mm':
                    $key=array
                    ('ÝÑæÑÏ?ä','ÇÑÏ?ÈåÔÊ','ÎÑÏÇÏ','Ê?Ñ','ãÑÏÇÏ','ÔåÑ?æÑ','ãåÑ','ÂÈÇä','ÂÐÑ','Ï?','Èåãä','ÇÓÝäÏ');
                    $array[$type]=$key[$num-1];
                    break;

                case'rr':
                    $key=array('?˜','Ïæ','Óå','åÇÑ','äÌ','ÔÔ','åÝÊ','åÔÊ','äå','Ïå','?ÇÒÏå','ÏæÇÒÏå','Ó?ÒÏå',
                        'åÇÑÏå','ÇäÒÏå','ÔÇäÒÏå','åÝÏå','åÌÏå','äæÒÏå','È?ÓÊ','È?ÓÊ æ ?˜','È?ÓÊ æ Ïæ','È?ÓÊ æ Óå',
                        'È?ÓÊ æ åÇÑ','È?ÓÊ æ äÌ','È?ÓÊ æ ÔÔ','È?ÓÊ æ åÝÊ','È?ÓÊ æ åÔÊ','È?ÓÊ æ äå','Ó?','Ó? æ ?˜');
                    $array[$type]=$key[$num-1];
                    break;

                case'rh':
                    $key=array('?˜ÔäÈå','ÏæÔäÈå','Óå ÔäÈå','åÇÑÔäÈå','äÌÔäÈå','ÌãÚå','ÔäÈå');
                    $array[$type]=$key[$num];
                    break;

                case'sh':
                    $key=array('ãÇÑ','ÇÓÈ','æÓÝäÏ','ã?ãæä','ãÑÛ','Ó','Îæ˜','ãæÔ','Çæ','áä','ÎÑæÔ','äåä');
                    $array[$type]=$key[$num%12];
                    break;

                case'mb':
                    $key=array('Íãá','ËæÑ','ÌæÒÇ','ÓÑØÇä','ÇÓÏ','ÓäÈáå','ã?ÒÇä','ÚÞÑÈ','ÞæÓ','ÌÏ?','Ïáæ','ÍæÊ');
                    $array[$type]=$key[$num-1];
                    break;

                case'ff':
                    $key=array('ÈåÇÑ','ÊÇÈÓÊÇä','Ç??Ò','ÒãÓÊÇä');
                    $array[$type]=$key[(int)($num/3.1)];
                    break;

                case'km':
                    $key=array('ÝÑ','ÇÑ','ÎÑ','Ê?ž','ãÑ','Ôåž','ãåž','ÂÈž','ÂÐ','Ï?','Èåž','ÇÓž');
                    $array[$type]=$key[$num-1];
                    break;
    ...
    public function gregorian_to_jalali($g_y,$g_m,$g_d,$mod=''){
        $g_y=$this->tr_num($g_y); $g_m=$this->tr_num($g_m); $g_d=$this->tr_num($g_d);/* <= :Çíä ÓØÑ ¡ ÌÒÁ ÊÇÈÚ ÇÕáí äíÓÊ */
        $d_4=$g_y%4;
    ...
    public function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
        $j_y=$this->tr_num($j_y); $j_m=$this->tr_num($j_m); $j_d=$this->tr_num($j_d);/* <= :Çíä ÓØÑ ¡ ÌÒÁ ÊÇÈÚ ÇÕáí äíÓÊ */
        $d_4=($j_y+1)%4;
